...
def calculate_foreground_weight(data_dir, project_name, train_val_name, mode, one_hot):
    ...
    :return:
    """
    print("Calculating foreground weight...")
    instance_names = []
...
def calculate_min_object_size(data_dir, project_name, train_val_name, mode, one_hot):
    ...
    :return:
    """
    print("Calculating minimum object size...")
    instance_names = []
    size_list = []
    for name in train_val_name:
        instance_dir = os.path.join(data_dir, project_name, name, 'masks')
...
def calculate_max_eval_image_size(data_dir, project_name, test_name, mode, one_hot):
    print("Calculating max_eval_image_size...")
    instance_names = []
    size_z_list = []
    size_y_list = []
    size_x_list = []
...
def calculate_avg_background_intensity(data_dir, project_name, train_val_name, one_hot):
    ...
    :return:
    """
    print("Calculating avg_background_intensity...")
    instance_names = []
    image_names = []
    for name in train_val_name:
        instance_dir = os.path.join(data_dir, project_name, name, 'masks')
...
def get_data_properties(data_dir, project_name, train_val_name, test_name, mode, one_hot): 
    ...
    """
    data_properties_dir = {}
    data_properties_dir['foreground_weight'] = calculate_foreground_weight( data_dir, 
                                                                            project_name, 
                                                                            train_val_name, 
                                                                            mode,
                                                                            one_hot)
    data_properties_dir['min_object_size'] = calculate_min_object_size( data_dir, 
                                                                        project_name, 
                                                                        train_val_name, 
                                                                        mode,
                                                                        one_hot).astype(np.float)
    data_properties_dir['n_z'], data_properties_dir['n_y'], data_properties_dir['n_x'] = calculate_max_eval_image_size( data_dir, 
                                                                                                                        project_name, 
                                                                                                                        test_name, 
                                                                                                                        mode, 
                                                                                                                        one_hot) 
